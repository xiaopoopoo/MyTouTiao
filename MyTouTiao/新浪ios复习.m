...
系统默认将kCFRunLoopDefaultMode和UITrackingRunLoopMode添加到了_commonModes中，即标识为Common属性，
所以当RunLoop运行在这两种模式中会自动同步添加_commonModeItems中的Source/Timer/Observer。
代码：
...
想控制runloop的退出时机，而不是在处理完一个输入源事件之后就退出，那么就要重复调用runMode:beforeDate:，
退出使用CFRunLoopStop(CFRunLoopGetCurrent());
...
常驻线程，创建一个线程，回调方法中创建事件源，model，添加给runloop，while循环bool变量yes
，中不断去运行这个事件源，在while循环外，移除runloop事件源，release事件源。
...
上面的栗子很好理解，主要利用了一个source1事件的监听，由于Mode的Source/Observer/Timer中的Observer不为空，所以RunLoop不会退出循环，
能够常驻内存。
...
超文本传输协议
  ...
  响应报文：
    ...
    1 客户端向server发送报文，客户端支持的tls协议版本， 支持的加密算法， 一个随机数c
    2 server端返回一个握手报文消息包含后面内容：商定的加密算法（我选择哪个客户端支持的算法），随机数s，
      server证书(证书中包含了网站的地址，加密用的公钥，以及证书的颁发机构等) 而私钥则服务器保存
    3 客户端收到内容后 验证server证书，及对证书的一个公钥进行验证，判断当前server是否是一个合法的server
     (证书是用来标示一个站点是否合法的标志。如果说该证书由权威的第三方颁发和签名的，则说明证书合法)
    4 如果证书合法，或者客户端接受和信任了不合法的证书,客户端组装会话秘钥(一串序列号)，通过随机数c，s包括客户端产生的一个预主密钥
     (估计是服务器返回的公钥证书)来进行会话密钥组装
    5 客户端再发送一个报文到server端，通过server端的公钥对预主密钥进行加密传输
    6 server端通过私钥解密得到预主密钥
    7 server端通过随机数c,s,包括通过私钥解密得到预主密钥组装会话密钥
    8 从客户端发送一个加密的握手消息
    9 server返回给客户端加密的握手消息，来验证我们的安全通道是否建立完成
    ...
    下面这个https的说法更通俗理解：
    简化版
客户端发请求服务器，服务器最开始有私钥和公钥，私钥保存，只把公钥放到一个证书中，这个证书是受保护空间中的，
证书是购买的，客户端需要选择是否安装这个证书，并信任它，如
果客户端信任，就建立了一个受保护层，服务器就可以在这个保护层把数据传给客户端
这个证书先客户端安装，安装后传的请求参数都通过这个证书的公钥进行了加密，发给服务器，服务器通过自己的私匙解密，
然后把返回的数据通过私钥加密，发给客户端
...
，把这个随机数，还有公钥放到证收中，传给客户端，（像银行需要确认客户是否安全，
 这里可以发送一个正式客户提供USB密钥，里面就包含了一张客户端证书。密钥对了才能继续操作）
3、客户端安装信任证书后，再生成一个随机数，加上之前生成的随机数，和服务器发过来的随机数，结合这个证书的公钥
把请求参数加密，发给服务器，
4、服务器根据三个随机数，和私钥进行解密，再把返回的数据通过三个随机数，和私钥加密返回给客户端，
(这里握手成功后一般可以用对称加密处理，因为已经认证了安全性，所以对称加密耗时更少)
5、客户端再通过三个随机数和证书中的
公钥解密数据。
...
服务器端会向CA申请认证书，此证书包含了CA及服务器端的一些信息（可以理解为类似公章），这样，
服务器端将证书发给客户端的过程中，中间方是无法伪造的，保证了，发给客户端的公钥是服务器端发送的。
...
HTTP特点：
 ...
 HTTP/1.1持久连接（HTTP keep-alive）方法：任意一端没有明确提出断开连接，则在一定时间内保持TCP连接状态，
 请求首部字段中的Connection: keep-alive即为表明使用了持久连接
...
传输层协议 ：
   ...
   特点：无连接  发送udp数据报不需要建立好连接，不需要释放连接
        ...
        udp用户数据报协议提供的功能有哪些呢？
          ...
          分用
            接收方通过ip接收了服务器传来的数据，然后拆成多个udp用户数据报，每个数据报中都有源端口和目的端口的标识概念，
            根据目的端口来分发数据，是分给端口1 或端口2，也就是回传给服务器上不同的应用
            在tcp上同样存在这种复用，分用的功能
          差错检测
...
tcp的特点：
    面向连接：
       ...
       客户端会重新发送syn请求同步报文
       服务端收到请求syn同步报文后，会发送同步应答报文，如果在这里又收到一个syn请求同步报文，服务端认为客户端想建立两次连接，
       但客户端其实是只想一次连接这个时候， server端发送两次同步确认报文syn ack，经过一段时间，客户端并没有传递客户端两次同步确认报文，
       这里候服务端就知道客户端并不是想要建立两次连接，因为第三次握手客户端只确认了一次连接  
...
图片缓存中线程的处理
图片通过url编码后的key经由manager 去memory中查找，
命中返回，未命中，通过磁盘缓存读 cache disk，如果有返回，并放入内存方便下次使用，如果没有发送 cache network download，并放到内存当。
